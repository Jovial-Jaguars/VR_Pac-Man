...
export default class MazeRunner extends React.Component {
  ...
  getSuccessors(coordz, coordx, arr) {
    ...
    if (arr[coordz + 1] !== undefined) {
      if (
        arr[coordz + 1][coordx] !== 1 &&
        arr[coordz + 1][coordx] !== 4 &&
        arr[coordz + 1][coordx] !== undefined
      ) {
        suc.push([[coordz + 1, coordx], "S"]);
      }
    }
    if (
      arr[coordz][coordx - 1] !== 1 &&
      arr[coordz][coordx - 1] !== 4 &&
      arr[coordz][coordx - 1] !== undefined
    ) {
  ...
  path(start, arr) {
    ...
    do {
      ...
      for (var suc in Successors) {
        ...
        if (!(calc in recur)) {
          q.unshift([
            [Successors[suc][0][0], Successors[suc][0][1]],
            curr[1] + Successors[suc][1],
          ]);
  ...
  createSphereBody(position, radius, id, type) {
    ...
    var pelletBody = new CANNON.Body({
      mass: 0,
      position: new CANNON.Vec3(position.x, position.y, position.z),
      shape: pelletShape,
    });
  ...
  createGhostBody(ghostPosition, id) {
    ...
    ghostBody.quaternion.setFromAxisAngle(
      new CANNON.Vec3(1, 0, 0),
      -Math.PI / 2
    );
    ghostBody.id = id;
    ghostBody.addEventListener(
      "collide",
      function (ghostBody, e) {
        if (e.body.isPlayer && !this.isGameOver) {
          if (this.isGhostPellet && this.ghostRemover === 0) {
            this.score += 250;
            this.scoreboard.children[0].text =
            ...
            scores.push(
              new BABYLON.Text2D(this.score.toString(), {
                id: "text4",
                y: -60,
            ...
            // do a get request to get highscores
            // do a post request to store highscore
            scores.push(
            ...
            gameOver.children[3].pointerEventObservable.add(
              function () {
                this.score = 0;
                this.engine.stopRenderLoop();
                this.engine.clear(BABYLON.Color3.Black(), false, false);
            ...
            gameOver.children[4].pointerEventObservable.add(
              function () {
                this.engine.stopRenderLoop();
                this.engine.clear(BABYLON.Color3.Black(), false, false);
    ...
    for (var i = 0; i < arr.length; i++) {
      for (var j = 0; j < arr[i].length; j++) {
        if (arr[i][j] === 1) {
          ///if theres a wall
          if (flipMaze) {
            //if theres an upside down maze, gravity switch
            var newInstanceWall = wall.createInstance(
              flipMaze + "block j" + i * 16 + j
            ); //creates instance/copy  of a single block as many times needed
            newInstanceWall.position.z = z;
            newInstanceWall.position.x = x;
            newInstanceWall.position.y = 900; // set positions relative to array
            this.createWallBody(
              {
                x: newInstanceWall.position.x,
                y: newInstanceWall.position.y,
                z: newInstanceWall.position.z,
              },
              new CANNON.Vec3(wall.scaling.x, wall.scaling.y, wall.scaling.z),
              0
            ); /// create cannon physics version of block with the same size
          } else {
            ...
            this.createWallBody(
              ...
              new CANNON.Vec3(wall.scaling.x, wall.scaling.y, wall.scaling.z),
              0
            ); //t gives block physics in cannon world
          }
        } else if (
          arr[i][j] === 2 ||
          arr[i][j] === 5 ||
          ...
          // same thing but for pellets
          if (flipMaze) {
            if (arr[i][j] === 2) {
              var newInstanceSphere = pellet.createInstance(
                flipMaze + "pellet i" + i * 16 + j
              );
              ...
              this.createSphereBody(
                {
                  x: newInstanceSphere.position.x,
                  y: newInstanceSphere.position.y,
                  z: newInstanceSphere.position.z,
                },
                6,
                flipMaze + "" + i * 16 + j,
                "normal"
              ...
              var newInstanceSphere = pellet.clone(
                flipMaze + "gravity i" + i * 16 + j
              );
              newInstanceSphere.position.z = z;
              newInstanceSphere.position.x = x;
              newInstanceSphere.position.y = 995;
              newInstanceSphere.material = new BABYLON.StandardMaterial(
                "gravity1",
                scene
              );
  ...
  switchCamera(scene, cam, canvas) {
    //copy ove all of current camera parameters to new camera either vr or 3d view
  ...
  createScene(canvas) {
    ...
    this.scoreboard.children[1].pointerEventObservable.add(
      function (scene, canvas) {
        window.scrollTo(0, 1);
        if (
          scene.activeCameras[0] instanceof BABYLON.FreeCamera &&
          !(
            scene.activeCameras[0] instanceof
            BABYLON.VRDeviceOrientationFreeCamera
          )
        ) {
          //if 3d camera
          this.camera = new BABYLON.VRDeviceOrientationFreeCamera(
            "deviceOrientationCamera",
            scene.activeCameras[0].position,
            scene
          );
      ...
      }.bind(this, scene, canvas),
      BABYLON.PrimitivePointerInfo.PointerUp
    );

    var hl = new BABYLON.HighlightLayer("hl1", scene);
    var pacTask = this.assetsManager.addMeshTask(
      "pacman task",
      "",
      "../assets/",
      "pac.babylon"
      ...
      meshTask.onSuccess = function (task) {
        ...
        for (var i = 0; i < task.loadedMeshes.length; i++) {
          ...
          if (ghosty.name === "Plane") {
            ...
            ghosty.name === "Sphere.002" ||
            ghosty.name === "Sphere.003"
          ) {
            ghosty.material = new BABYLON.StandardMaterial("ghosty", scene);
        ...
        if (this.isGhostTwo) {
          ...
          this.ghosts[1].mesh.material = new BABYLON.StandardMaterial(
            "ghostPellet2",
            scene
          );
        ...
        if (this.isGhostThree) {
          ...
          this.ghosts[2].mesh.material = new BABYLON.StandardMaterial(
            "ghostPellet3",
            scene
          );
    ...
    this.blockMesh.material.emissiveTexture = new BABYLON.Texture(
      "../assets/tron1.jpg",
      scene
    );
    ...
    this.createWallBody(
      { x: 0, y: 0, z: 0 },
      new CANNON.Vec3(plane.scaling.x, plane.scaling.y, plane.scaling.z),
      0
    );
    ...
    plane2.scaling.x = 0.2;
    plane2.position.x = 200;
    this.createWallBody(
      { x: plane2.position.x, y: 0, z: 0 },
      new CANNON.Vec3(plane2.scaling.x, plane2.scaling.y, plane2.scaling.z),
      0
    );
    var plane3 = plane.createInstance("i" + 302);
    plane3.scaling.z = 100;
    plane3.scaling.y = 1000;
    plane3.scaling.x = 0.2;
    plane3.rotation.y = Math.PI / 2;
    plane3.position.x = 100;
    plane3.position.z = 100;
    this.createWallBody(
      { x: plane3.position.x, y: 0, z: plane3.position.z },
      new CANNON.Vec3(plane3.scaling.x, plane3.scaling.y, plane3.scaling.z),
      2
    );
  ...
  createWorld() {
    ...
    this.pacmanBody = new CANNON.Body({
      mass: mass,
      position: new CANNON.Vec3(
        this.pacmanPosition.x,
        this.pacmanPosition.y,
        this.pacmanPosition.z
      ),
      shape: sphereShape,
    }); // Step 2
    this.pacmanBody.rotation = new CANNON.Vec3();
    this.pacmanBody.quaternion.setFromAxisAngle(
      new CANNON.Vec3(1, 0, 0),
      -Math.PI / 2
    );
    this.pacmanBody.isPlayer = true;
    this.pacmanBody.addEventListener(
      "collide",
      function (e) {
        if (e.body.isPellet) {
  ...
  runLoop() {
    ...
    if (this.cameraFlag && this.camera.rotationQuaternion !== undefined) {
      this.pacmanBody.velocity.z =
        this.pacVelocity *
        parseFloat(Math.cos(this.camera.rotationQuaternion.toEulerAngles().y));
      this.pacmanBody.velocity.x =
        this.pacVelocity *
        parseFloat(Math.sin(this.camera.rotationQuaternion.toEulerAngles().y));
    } else {
      this.pacmanBody.velocity.z =
        this.pacVelocity * parseFloat(Math.cos(this.camera.rotation.y));
      this.pacmanBody.velocity.x =
        this.pacVelocity * parseFloat(Math.sin(this.camera.rotation.y));
    }
    ...
    if (this.camera.position.y > 950) {
      this.camera.position.x = this.pacmanBody.position.x + 0.4;
    ...
    if (
      Math.abs(Math.floor(this.camera.position.x / 12.5)) !==
        this.currPacVec.j ||
      Math.abs(
        Math.floor((this.camera.position.z - 87.5) / 12.5) !== this.currPacVec.i
      )
    ) {
      this.maze[this.currPacVec.i][this.currPacVec.j] = 0;
      this.currPacVec.j = Math.abs(Math.floor(this.camera.position.x / 12.5));
      this.currPacVec.i = Math.abs(
        Math.floor((this.camera.position.z - 87.5) / 12.5)
      );
      this.maze[this.currPacVec.i][this.currPacVec.j] = 3;
    }
    if (this.isGhostOne && !this.isGameOver) {
      if (
        Math.abs(Math.floor(this.ghosts[0].body.position.x / 12.5)) !==
          this.ghosts[0].j ||
        Math.abs(
          Math.floor((this.ghosts[0].body.position.z - 87.5) / 12.5) !==
            this.ghosts[0].i
        )
      ) {
        this.maze[this.ghosts[0].i][this.ghosts[0].j] = 0;
        this.ghosts[0].j = Math.abs(
          Math.floor(this.ghosts[0].body.position.x / 12.5)
        );
        this.ghosts[0].i = Math.abs(
          Math.floor((this.ghosts[0].body.position.z - 87.5) / 12.5)
        );
    ...
    if (this.isGhostTwo && !this.isGameOver) {
      if (
        Math.abs(Math.floor(this.ghosts[1].body.position.x / 12.5)) !==
          this.ghosts[1].j ||
        Math.abs(
          Math.floor((this.ghosts[1].body.position.z - 87.5) / 12.5) !==
            this.ghosts[1].i
        )
      ) {
        this.maze[this.ghosts[1].i][this.ghosts[1].j] = 0;
        this.ghosts[1].j = Math.abs(
          Math.floor(this.ghosts[1].body.position.x / 12.5)
        );
        this.ghosts[1].i = Math.abs(
          Math.floor((this.ghosts[1].body.position.z - 87.5) / 12.5)
        );
    ...
    if (this.isGhostThree && !this.isGameOver) {
      if (
        Math.abs(Math.floor(this.ghosts[2].body.position.x / 12.5)) !==
          this.ghosts[2].j ||
        Math.abs(
          Math.floor((this.ghosts[2].body.position.z - 87.5) / 12.5) !==
            this.ghosts[2].i
        )
      ) {
        this.maze[this.ghosts[2].i][this.ghosts[2].j] = 0;
        this.ghosts[2].j = Math.abs(
          Math.floor(this.ghosts[2].body.position.x / 12.5)
        );
        this.ghosts[2].i = Math.abs(
          Math.floor((this.ghosts[2].body.position.z - 87.5) / 12.5)
        );
  ...
  componentDidMount() {
    ...
    if (this.isGhostOne) {
      this.ghosts[0].interval = setInterval(
        function () {
          this.ghosts[0].directions = this.path(
            [
              Math.abs(
                Math.floor((this.ghosts[0].body.position.z - 87.5) / 12.5)
              ),
      ...
      this.ghosts[0].directions = this.path(
        [
          Math.abs(Math.floor((this.ghosts[0].body.position.z - 87.5) / 12.5)),
          Math.abs(Math.floor(this.ghosts[0].body.position.x / 12.5)),
        ],
        this.maze
      ).split("");
    ...
    if (this.isGhostTwo) {
      this.ghosts[1].interval = setInterval(
        function () {
          this.ghosts[1].directions = this.path(
            [
              Math.abs(
                Math.floor((this.ghosts[1].body.position.z - 87.5) / 12.5)
              ),
      ...
      this.ghosts[1].directions = this.path(
        [
          Math.abs(Math.floor((this.ghosts[1].body.position.z - 87.5) / 12.5)),
          Math.abs(Math.floor(this.ghosts[1].body.position.x / 12.5)),
        ],
        this.maze
      ).split("");
    ...
    if (this.isGhostThree) {
      this.ghosts[2].interval = setInterval(
        function () {
          this.ghosts[2].directions = this.path(
            [
              Math.abs(
                Math.floor((this.ghosts[2].body.position.z - 87.5) / 12.5)
              ),
      ...
      this.ghosts[2].directions = this.path(
        [
          Math.abs(Math.floor((this.ghosts[2].body.position.z - 87.5) / 12.5)),
          Math.abs(Math.floor(this.ghosts[2].body.position.x / 12.5)),
        ],
        this.maze
      ).split("");
