...
componentWillMount(){
  ...
  showPublicMaps(){
    this.setState({showMaps: 'publicMaps', sidebarOpen: false});

  }

  showCartMaps() {
    this.setState({showMaps: 'cartMaps', sidebarOpen: false});
  ...
  sendUpdate (){
    var apiPackage = [];
    for (var key in this.state.updateMyMaps){
      var oneMap = {};
      var mapDataText = '';
      for (var i = 0; i < this.state.myMaps[key][0].length; i++){
        for (var j= 0; j < this.state.myMaps[key][0][i].length; j++){
